...
export const CurrentVisitContainer = (props: Props) => {
  const [observationToBeLiftedVisible, setObservationToBeLiftedVisible] =
    useState(false);
  ...
  const [addNewObservationVisible, setAddNewObservationVisible] =
    useState(false);
  ...
  const ObservationToBeLiftedTitle =
    t('txt.my.remarks') +
    ' ' +
    '(' +
    observationsToBeLifted.length.toString() +
    ')';
  ...
  const content = [
    {
      type: 'image',
      source: utils.images.icn_delete,
      onPress: () => {
        DeleteVisit();
      },
    },
    {
      type: 'image',
      source: utils.images.icn_edit,
      onPress: () => editCurrentVisit(),
    },
  ];
  ...
  const {
    ...
    selectedSite,
    selectedSiteRef,
  } = route.params; // Access the parameters
  ...
  return (
    <View style={styles.container}>
      ...
      <ObservationList
        ...
        NumObservation={observations.length}
        NumObservationTobeLifted={observationsToBeLifted.length}
        type={type}
        onClickEssentialActions={() => setObservationVisible(true)}
        NumEssentialActions={observations.length}
        onClickStrongPoints={() => setStrongPointsVisible(true)}
        onClickPointsToImprove={() => setPointsToImproveVisible(true)}
        NumStrongPoints={0}
        NumPointsToImprove={0}
      />
      <BottomFooter
        confirmPress={() => saveVisit()}
        confirmText={t('flash_alert_button')}
        content={content}
      />
      <ObservationToBeLiftedModal
        visible={observationToBeLiftedVisible}
        onClose={() => setObservationToBeLiftedVisible(false)}
        title={ObservationToBeLiftedTitle}
        observationToBeLifted={observationsToBeLifted}
      />
      <ObservationModal
        visible={observationVisible}
        onClose={() => setObservationVisible(false)}
        title={t('txt.title.ajout.observations')}
        observation={observations}
        setobservations={setobservations}
        filtedObservations={observation}
        AddNewObservationModal={() => setAddNewObservationVisible(true)}
        setObservationTitle={setobservationTitle}
      />
      <AddNewObservationModal
        visible={addNewObservationVisible}
        onClose={() => setAddNewObservationVisible(false)}
        title={t('txt.title.ajout.observations')}
        observationTitle={observationTitle.toString()}
        titleComment={titleComment}
        setTitleComment={setTitleComment}
        comment={comment}
        ...
        idRemarque={generateID()}
        idVisits={visit.id}
      />
      <StrongPointsModal
        visible={strongPointsVisible}
        onClose={() => setStrongPointsVisible(false)}
        title={StrongpointsTitle}
        StrongPointsList={[]}
        AddNewStrongPointsModal={() =>
          console.log('Add New Strong Points Modal')
        }
      />
      <PointsToImproveModal
        visible={pointsToImproveVisible}
        onClose={() => setPointsToImproveVisible(false)}
        title={PointsToImproveTitle}
        StrongPointsList={[]}
        AddNewPointsToImproveModal={() =>
          console.log('Add New Points To Be Improve Modal')
        }
      />
    </View>
  );
