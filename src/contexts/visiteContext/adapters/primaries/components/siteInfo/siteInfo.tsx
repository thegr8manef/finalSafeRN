...
export const SiteInfo = (props: Props) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{t('selectionner_le_chantier_par')}</Text>
      <View style={styles.content}>
        <SearchSiteItem
          label={t('code_chantier_btn')}
          icon={utils.images.inputIcon}
          onPress={() => setSearchWithCodeVisible(true)}
        />
        <SearchSiteItem
          label={t('chantier_de_mon_perimetre')}
          icon={utils.images.constructionIcon}
          onPress={() => setSearchWithNameVisible(true)}
        />
        <SearchSiteItem
          label={t('qr_code')}
          icon={utils.images.qrCodeIcon}
          onPress={() => setSearchWithQrCodeVisible(true)}
        />
      </View>
      <PreviewSite site={props.selectedSite} />
      <SearchSiteWithCodeModal
        modalVisible={searchWithCodeVisible}
        onClose={() => setSearchWithCodeVisible(false)}
        onSearch={(site: Site | undefined) => props.setSelectedSite(site)}
        sites={props.sites}
      />
      <SearchSiteWithNameModal
      ...
      <SearchSiteWithQrCode
        ...
        onClose={() => setSearchWithQrCodeVisible(false)}
        sites={props.sites}
        onSearch={(site: Site | undefined) => props.setSelectedSite(site)}
      />
    </View>
