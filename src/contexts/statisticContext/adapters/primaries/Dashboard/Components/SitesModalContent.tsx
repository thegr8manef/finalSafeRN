...
export const SitesModalContent = (props: Props) => {
  ...
  const handleSelect = (site: Site | undefined) => {
    if (site) {
      if (filtredSites.includes(site)) {
        console.log(site);

        setSelectedSites(props.sites.filter(s => s.id !== site.id));
      } else {
        setSelectedSites([...filtredSites, site]);
      }
    }
  };
  ...
  const renderItem = ({
    ...
    return (
      <TouchableOpacity
        ...
        style={[styles.item, desactivated && {backgroundColor:utils.colors.gray90}]}>
