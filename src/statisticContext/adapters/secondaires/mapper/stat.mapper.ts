...
export class StatMapper {
  static mapStatisticToStat(data: Statistic): Stat {
    const risk0 = { label: data.statisticRisk?.risk0?.to, value: data.statisticRisk?.risk0?.pct }
    const risk1 = { label: data.statisticRisk?.risk1?.to, value: data.statisticRisk?.risk1?.pct }
    const risk2 = { label: data.statisticRisk?.risk2?.to, value: data.statisticRisk?.risk2?.pct }
    const otherRisk = { label: '', value: data.statisticRisk?.riskOthers }
    const statRisk = new StatRisk(risk0, risk1, risk2, otherRisk)
    const statVisit = new StatVisit(data.statisticVisit?.vf, data.statisticVisit?.vh, data.statisticVisit?.vc, data.statisticVisit?.vp, data.statisticVisit?.tv)
    const statObservation = new StatObservation(data.statisticObservation?.ol,
    ...
    const stat = new Stat(statRisk, statVisit, statObservation, statUser, data.date_synchro);
    return stat;
  }
